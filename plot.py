...
## this fuinction sets up the plots to look the same as in the astroML text
    ...
    ax3.set_title('Avg STRING Network Density')
    ...
    text_kwargs = dict(transform=plt.gca().transAxes,
                       ha='center', va='bottom')
    ax4.text(0.20, 1.02, 'Row scaling', color='k', **text_kwargs)
    ax4.text(0.50, 1.02, 'Mot scaling', color='r', **text_kwargs)
    ax4.text(0.80, 1.02, 'Net scaling', color='g', **text_kwargs)
    
    ax2a = fig.add_subplot(3,3,5, sharex=ax1)
    vals = stats[["iter", "ROWS"]].dropna(how="any").values
    ax2a.plot(vals[:, 0], vals[:, 1], **plot_kwargs)
    ax2a.set_title('Avg Num Genes')
    ax2a.set_ylabel('Iteration')
    ...
    text_kwargs = dict(transform=plt.gca().transAxes,
                       ha='center', va='bottom')
    ax4.text(0.20, 1.02, 'Rows', color='k', **text_kwargs)
    ax4.text(0.80, 1.02, 'Cols', color='r', **text_kwargs)
    ...
    text_kwargs = dict(transform=plt.gca().transAxes,
                       ha='center', va='bottom')
    ax5.text(0.20, 1.02, 'Num Moves', color='k', **text_kwargs)
    ax5.text(0.70, 1.02, 'Num Improvements', color='r', **text_kwargs)
    ...
    plt.hist( resids, 20 )
    ax5a.set_title('Residuals')
    ax5a.set_xlabel('Residual')
    ...
    plt.hist( pclusts, 20 )
    ax6.set_title('Cluster Mean Motif P-values')
